...
export const TransactionProvider = ({ children }) => {
  ...
  useEffect(() => {
    fetch("https://expense-tracker-7y00.onrender.com/transactions")
      .then((res) => res.json())
      .then((data) => {
  ...
  const addTransaction = async (tx) => {
    try {
      const res = await fetch("https://expense-tracker-7y00.onrender.com/transactions", {
        method: "POST",
  ...
  const deleteTransaction = async (_id) => {
    try {
      await fetch(`https://expense-tracker-7y00.onrender.com/transactions/${_id}`, {
        method: "DELETE",
      });
  ...
  const editTransaction = async (_id, tx) => {
    try {
      const updatedTx = await fetch(
        `https://expense-tracker-7y00.onrender.com/transactions/${_id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(tx),
